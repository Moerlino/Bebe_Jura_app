...
function header(sub = 'Europarecht I', showHome = state.screen !== 'home') {
  const homeButton = showHome ? `<button class="home-btn" id="homeButton" aria-label="Zur Startseite">Start</button>` : '';
  return `<div class="brand"><div><p class="kicker">${sub}</p><h2>Bebe Jura</h2></div><div class="brand-actions">${homeButton}<div class="logo">B</div></div></div>`;
}
function bindHomeButton() {
  const homeButton = document.querySelector('#homeButton');
  if (homeButton) homeButton.onclick = goHome;
}
function renderHome() {
  state.screen = 'home';
